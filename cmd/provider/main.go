...
import (
	...
	"github.com/gaarutyunov/provider-slicervm/apis"
	slicervm "github.com/gaarutyunov/provider-slicervm/internal/controller"
	"github.com/gaarutyunov/provider-slicervm/internal/version"
)

func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "Slicer VM support for Crossplane.").DefaultEnvars()
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-slicervm",
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Slicer APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-slicervm:%s", version.Version))),
		}
		o.ChangeLogOptions = &clo
	}

	kingpin.FatalIfError(customresourcesgate.Setup(mgr, o), "Cannot setup CRD gate controller")
	kingpin.FatalIfError(slicervm.SetupGated(mgr, o), "Cannot setup Slicer controllers")
